...
import { UIClassDAO, FieldsAndMethods } from "./DAO/UIClassDAO";
...
export class SyntaxAnalyzer {
	...
	private static getClassNameFromVariableParts(variableParts: string[], className: string, usedPartQuantity: number = 2) : string | undefined {
		...
		let UIClass = <CustomUIClass>UIClassDAO.getUIClass(className);
		let UIClassName = UIClassDAO.getClassOfTheVariableHierarchically(variableString, UIClass);
	...
	private static setNewContentForCurrentUIClass() {
		if (vscode.window.activeTextEditor) {
			...
			if (currentClassName) {
				...
				UIClassDAO.setNewCodeForClass(currentClassName, documentText);
			} else {
				debugger;
			}
		}
	...
	private static getFieldsAndMethodsFor(variable: string, className: string, position: number = 0) {
		...
		if (vscode.window.activeTextEditor) {
			fieldsAndMethods = UIClassDAO.getFieldsAndMethodsForVariable(variable, className, position);
		}
		return fieldsAndMethods;
	}
